(function (context) {
  ...
  TheGraph.Port = React.createFactory( React.createClass({
    ...
    edgeStart: function (event) {
      ...
      // We only want the trackstart event
      if (event.type === 'track' && event.detail.state !== 'start') {
        return;
      }
    ...
    triggerDropOnTarget: function (event) {
      // Related target is not set by default
      if (event.detail.sourceEvent) {
        event.relatedTarget = event.detail.sourceEvent.target;
      }
      // If dropped on a child element will bubble up to port
      if (!event.relatedTarget) { return; }
      // We only want trackend here
