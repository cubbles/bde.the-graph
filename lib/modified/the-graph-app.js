(function (context) {
  ...
  TheGraph.config.app = {
    container: {
      className: "the-graph-app style-scope the-graph",
      name: "app"
    },
    canvas: {
      ref: "canvas",
      className: "app-canvas style-scope the-graph"
    },
    svg: {
      className: "app-svg style-scope the-graph"
    },
    svgGroup: {
      className: "view style-scope the-graph"
    },
    graph: {
      ref: "graph"
    },
    ...
    modal: {
      className: "context style-scope the-graph"
    }
  };
  ...
  TheGraph.App = React.createFactory( React.createClass({
    ...
    onTrack: function(event) {
      ...
      // Don't propagate event to Elements beyond this
      event.stopPropagation();
      event.stopImmediatePropagation();
      ...
      this.setState({
          x: this.state.x + (event.ddx || 0),
          y: this.state.y + (event.ddy || 0)
      });
    },
